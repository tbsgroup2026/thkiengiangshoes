    ...
    if (url.pathname.startsWith("/api/ci-kaizen")) {
      ...
      if (request.method === "GET") {
        try {
          ...
          let whereSql = " WHERE 1=1 AND (region LIKE '%Kiên Giang%' OR region LIKE '%KG%' OR factory LIKE '%Kiên Giang%' OR factory LIKE '%KG%' OR plant_code LIKE '%KG%' OR code LIKE 'KZ-%-KG%')";
          const params = [];
      ...
      if (request.method === "POST") {
        try {
          ...
          const code = await generateRecordCode(env, {
            ...
            workshop: department || "Xưởng Đế KG1",
            year: proposerYear || new Date().getFullYear()
          });
